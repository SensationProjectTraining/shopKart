    ...
    <div>
      ...
      {Isloading ? Array.from({length:6}).map((_,index)=>(
        ...
        <div key={index} className="bg-white h-auto w-[250px]  rounded-xl p-3">
          ...
          <Skeleton height={30} width="100%" className='mb-2'></Skeleton><hr/>
          <Skeleton height={20} width="100%" className='mb-2  mt-4'></Skeleton>
          <Skeleton height={20} width="100%" className='mb-2'></Skeleton>
          <Skeleton height={20} width="40%" className='mb-2 mt-1 inline-block'></Skeleton>
          
        </div>
      )) : (
          storedata.map((product) => (
            <div key={product.id} className="bg-white hover:scale-105 duration-300 ease-linear h-auto w-[250px] mt-5 mb-5 rounded-xl shadow-lg shadow-yellow-300 p-3">
              <img src={product.image} alt="image not found" className="h-[130px] w-full border-2 border-gray-500 p-2 rounded-md" />
              
              {/* Truncate the title */}
              <h1 className="text-center text-lg font-semibold mt-2 text-blue-800 hover:text-red-600">
                <Link to={`/product/${product.id}`}>
                {truncateText(product.title, 20)} {/* Truncate title to 30 characters */}
                </Link>
              </h1>
            <hr className='mt-2'/>
              {/* Truncate the description */}
              <p className="mt-3">{truncateText(product.description, 75)} {/* Truncate description to 100 characters */}</p>
